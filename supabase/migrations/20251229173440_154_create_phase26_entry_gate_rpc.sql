/*
  # Phase 26 Entry Gate RPC (Phase 26)

  ## Purpose
  Checks if user can enter Phase 26 (Secure Messaging & Agency Communications).
  Enforces hard gate conditions.

  ## Entry Conditions (ALL must be satisfied)
  1. Organization state = ACTIVE
  2. Phases 18-25 = COMPLETED
  3. User role ∈ {CAREGIVER, SUPERVISOR, AGENCY_ADMIN}

  ## Function
  - check_phase26_entry_gate() - Returns allow/block with detailed reasons
...
-- Function: check_phase26_entry_gate
-- Checks if current user can enter Phase 26
CREATE OR REPLACE FUNCTION check_phase26_entry_gate()
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Check user role (CAREGIVER, SUPERVISOR, AGENCY_ADMIN allowed)
  ...
  -- Phases 19-25 are implicitly completed if organization is active
