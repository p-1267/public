/*
  # Fix WP8 Showcase Seeder - Match vital_signs Schema
  
  vital_signs has:
  - vital_type (text)
  - value (text - not numeric!)
  - systolic/diastolic (integer) for blood pressure
  NO unit or source columns
*/
...
BEGIN
  ...
  FOR i IN 1..2 LOOP
    ...
    -- Create vitals from device data (match vital_signs schema)
    -- Heart rate
    INSERT INTO vital_signs (resident_id, vital_type, value, recorded_at)
    VALUES (v_resident_ids[1], 'heart_rate', (72 + (i * 3))::text, now() - interval '1 hour' + (i * interval '15 minutes'));
    
    -- Blood pressure (use systolic/diastolic columns)
    INSERT INTO vital_signs (resident_id, vital_type, value, systolic, diastolic, recorded_at)
    VALUES (v_resident_ids[1], 'blood_pressure', (120 + (i * 5))::text || '/' || (80 + (i * 2))::text, 120 + (i * 5), 80 + (i * 2), now() - interval '1 hour' + (i * interval '15 minutes'));
    
    -- Temperature
    INSERT INTO vital_signs (resident_id, vital_type, value, recorded_at)
    VALUES (v_resident_ids[1], 'temperature', (98.6 + (i * 0.2))::text, now() - interval '1 hour' + (i * interval '15 minutes'));
    
    -- SpO2
    INSERT INTO vital_signs (resident_id, vital_type, value, recorded_at)
    VALUES (v_resident_ids[1], 'spo2', (98 - i)::text, now() - interval '1 hour' + (i * interval '15 minutes'));

    -- Update staging record
    UPDATE device_data_staging
    SET vitals_created = 4
