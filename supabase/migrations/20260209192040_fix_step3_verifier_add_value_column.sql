/*
  # Fix Step 3 Verifier - Add value column to vital_signs insert
*/
...
DECLARE
  ...
  v_signal_count integer; v_timeline_count integer; v_idempotency_base text;
  v_scheduled_time timestamptz; v_administered_at timestamptz; v_time_diff_minutes integer;
BEGIN
  ...
  INSERT INTO vital_signs (resident_id, vital_type, value, systolic, diastolic, recorded_at, recorded_by, notes, is_simulation, idempotency_key) VALUES (v_test_resident_id, 'blood_pressure', 185.0, 185, 95, now(), v_test_user_id, 'Feeling pressure in head', true, v_idempotency_base || '_vital_1') RETURNING id INTO v_test_log_id;
  ...
  INSERT INTO observation_events (resident_id, observer_type, observer_id, event_type, observation_text, severity, quality, confidence_level, requires_followup, is_simulation) VALUES (v_test_resident_id, 'FAMILY', v_test_user_id, 'COGNITIVE', 'Mom seems confused and forgetful. She missed her morning medications.', 'MODERATE', 'HIGH', 0.85, true, true) RETURNING id INTO v_test_log_id;
