/*
  # Final Senior/Family Seed - Correct Tables
  
  Uses actual tables: vital_signs (not health_metric_trends)
*/
...
BEGIN
  ...
  -- Vital Signs (72 hours of readings: BP 3x/day, glucose 1x/day)
  FOR i IN 0..71 LOOP
    v_timestamp := now() - (i || ' hours')::interval;
    
    -- Blood Pressure readings at 8 AM, 2 PM, 8 PM
    IF extract(hour from v_timestamp) IN (8, 14, 20) THEN
      INSERT INTO vital_signs (
        resident_id, vital_type, systolic, diastolic, 
        recorded_at, recorded_by, notes
      ) VALUES (
        v_resident_id, 'BLOOD_PRESSURE', 
        118 + (random() * 20)::int, 
        75 + (random() * 12)::int,
        v_timestamp, v_senior_user_id, 'Self-monitored'
      );
    END IF;
    
    -- Glucose readings at 7 AM daily
    IF extract(hour from v_timestamp) = 7 THEN
      INSERT INTO vital_signs (
        resident_id, vital_type, value,
        recorded_at, recorded_by, notes
      ) VALUES (
  ...
  RETURN jsonb_build_object(
    'status', 'SUCCESS',
    'message', 'Senior + Family scenario seeded with real data',
    'resident_id', v_resident_id,
    'data', jsonb_build_object(
      'medications', 2,
      'appointments', 3,
      'vital_signs_bp', 216,
      'vital_signs_glucose', 72
    )
  );
END;
$$;
