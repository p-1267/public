/*
  # Final Seed Fix - All Case Issues
  
  Fixed: task categories to lowercase
*/
...
BEGIN
  ...
  IF v_context.management_mode IN ('SELF', 'FAMILY_MANAGED') THEN
    ...
    SELECT gen_random_uuid(), r.id, v_resident.full_name, true, v_agency_id FROM roles r WHERE r.name = 'SENIOR' ON CONFLICT DO NOTHING RETURNING id INTO v_senior_user_id;
  ...
  INSERT INTO resident_medications (resident_id, medication_name, dosage, dosage_unit, frequency, scheduled_time, status, prescribed_by) VALUES (v_resident.id, 'Lisinopril', '10', 'mg', 'Once daily', '09:00', 'ACTIVE', 'Dr. Sarah Johnson'), (v_resident.id, 'Metformin', '500', 'mg', 'Twice daily', '09:00', 'ACTIVE', 'Dr. Sarah Johnson') ON CONFLICT DO NOTHING;
  FOR i IN 0..4 LOOP
    INSERT INTO vital_signs (resident_id, measurement_type, value, unit, measured_at, measured_by, data_source) VALUES (v_resident.id, 'BLOOD_PRESSURE_SYSTOLIC', 120 + (i * 2), 'mmHg', now() - (i || ' days')::interval, v_caregiver_user_id, 'MANUAL'), (v_resident.id, 'HEART_RATE', 72 + i, 'bpm', now() - (i || ' days')::interval, v_caregiver_user_id, 'MANUAL') ON CONFLICT DO NOTHING;
    INSERT INTO health_metrics (resident_id, metric_category, metric_type, value, unit, recorded_at, data_source) VALUES (v_resident.id, 'VITAL', 'blood_pressure_systolic', 120 + (i * 2), 'mmHg', now() - (i || ' days')::interval, 'MANUAL'), (v_resident.id, 'VITAL', 'heart_rate', 72 + i, 'bpm', now() - (i || ' days')::interval, 'MANUAL') ON CONFLICT DO NOTHING;
  END LOOP;
  ...
  INSERT INTO resident_documents (resident_id, document_type, title, uploaded_by, file_path) VALUES (v_resident.id, 'CARE_PLAN', 'Care Plan', v_admin_user_id, '/documents/care-plan.pdf'), (v_resident.id, 'MEDICAL_RECORD', 'Medical History', v_admin_user_id, '/documents/medical-history.pdf') ON CONFLICT DO NOTHING;
