/*
  # Fix seed_active_context assigned_by NOT NULL

  Updates INSERT to include assigned_by field which is required.
  Uses supervisor or admin user if available, otherwise caregiver_user_id.
*/
...
BEGIN
  ...
  -- Create supervisor first if needed (for assigned_by reference)
  ...
  -- Now create caregiver and assignment
  IF v_context.service_model IN ('DIRECT_HIRE', 'AGENCY_HOME_CARE', 'AGENCY_FACILITY') THEN
    ...
    -- Use supervisor or admin as assigned_by, fallback to caregiver itself
