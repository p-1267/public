/*
  # Fix notification_log schema

  Changes:
  - Use recipient_user_id instead of user_id
  - Add required resident_id
  - Add required alert_type
  - Add required delivery_channels array
  - Use delivered_at instead of created_at for timestamp
  - Remove title field (doesn't exist)
  - Remove priority and status fields
*/
...
BEGIN
  ...
  VALUES
    ('DEVICE001', v_resident.id, 'BLE_HEALTH_SENSOR', 'BP Monitor', 'Omron', 'BP7200', '1.0.0', 'TRUSTED', COALESCE(v_caregiver_user_id, v_admin_user_id, v_senior_user_id))
  ON CONFLICT DO NOTHING;

  INSERT INTO notification_log (resident_id, recipient_user_id, notification_type, alert_type, message, delivery_channels, suppressed_by_preference, overridden_by_policy, delivered_at)
  VALUES
    (v_resident.id, COALESCE(v_family_user_id, v_senior_user_id, v_admin_user_id), 'health_alert', 'vital_sign', 'Recent BP reading shows elevation', ARRAY['IN_APP'], false, false, now() - interval '3 hours'),
    (v_resident.id, COALESCE(v_family_user_id, v_senior_user_id, v_admin_user_id), 'care_update', 'task_completion', 'Morning medication administered', ARRAY['IN_APP'], false, false, now() - interval '2 hours'),
    (v_resident.id, COALESCE(v_family_user_id, v_senior_user_id, v_admin_user_id), 'appointment_reminder', 'appointment', 'Checkup scheduled in 7 days', ARRAY['IN_APP'], false, false, now() - interval '1 hour')
  ON CONFLICT DO NOTHING;
