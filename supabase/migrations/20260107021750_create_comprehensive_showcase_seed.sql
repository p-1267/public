/*
  # Comprehensive Showcase Data Seeding

  Creates a single comprehensive RPC that seeds ALL showcase data including:
  - Agency and residents
  - Task categories, templates, and tasks
  - Intelligence signals, vital signs, and brain states
  - All with consistent UUID generation
*/
...
BEGIN
  ...
  RETURNING id INTO v_category_nursing;
  
  -- Get category IDs
  SELECT id INTO v_category_nursing FROM task_categories WHERE agency_id = v_agency_id AND name = 'NURSING';
  SELECT id INTO v_category_housekeeping FROM task_categories WHERE agency_id = v_agency_id AND name = 'HOUSEKEEPING';
  SELECT id INTO v_category_kitchen FROM task_categories WHERE agency_id = v_agency_id AND name = 'KITCHEN';
  SELECT id INTO v_category_hygiene FROM task_categories WHERE agency_id = v_agency_id AND name = 'HYGIENE';
  ...
  SELECT id INTO v_template_morning_med FROM task_templates WHERE agency_id = v_agency_id AND template_name = 'Morning Medication';
  SELECT id INTO v_template_vitals FROM task_templates WHERE agency_id = v_agency_id AND template_name = 'Vital Signs Check';
  SELECT id INTO v_template_room_clean FROM task_templates WHERE agency_id = v_agency_id AND template_name = 'Room Cleaning';
  SELECT id INTO v_template_breakfast FROM task_templates WHERE agency_id = v_agency_id AND template_name = 'Breakfast Delivery';
  ...
  -- 7. Create Vital Signs for first resident
