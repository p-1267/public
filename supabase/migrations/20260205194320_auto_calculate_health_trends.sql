/*
  # Auto-Calculate Health Trends in Seed

  Updates the seed_senior_family_scenario function to automatically
  calculate health_metric_trends after inserting health_metrics.

  This ensures trends are ALWAYS available when health data exists.
*/
...
BEGIN
  ...
  INSERT INTO senior_resident_links (senior_user_id, resident_id, relationship_type)
  VALUES (v_senior_user_id, v_resident_id, 'SELF')
  ...
  VALUES (v_family_user_id, v_resident_id, 'SON')
  ON CONFLICT (family_user_id, resident_id) DO NOTHING;

  INSERT INTO resident_medications (resident_id, medication_name, dosage, frequency, route, is_active, start_date)
  VALUES
    (v_resident_id, 'Lisinopril', '10mg', 'DAILY', 'ORAL', true, CURRENT_DATE - interval '90 days'),
    (v_resident_id, 'Metformin', '500mg', 'TWICE_DAILY', 'ORAL', true, CURRENT_DATE - interval '180 days')
  ON CONFLICT (resident_id, medication_name) DO UPDATE SET is_active = EXCLUDED.is_active;
  ...
  ON CONFLICT (resident_id, contact_name) DO UPDATE SET is_primary_contact = EXCLUDED.is_primary_contact;
  ...
  IF v_device_id IS NULL THEN
    SELECT id INTO v_device_id FROM device_registry
    WHERE device_id = 'OMRON-BP-' || substr(md5(v_resident_id::text), 1, 8) LIMIT 1;
  END IF;
