/*
  # Fix seed to handle NULL agency_id (SELF mode)

  Changes:
  - Skip agency-dependent inserts when agency_id IS NULL
  - Only seed Senior/Family roles and their data in SELF mode
  - Seed full agency structure when agency_id IS NOT NULL
*/
...
BEGIN
  ...
  -- Create user profiles (context-aware)
  IF v_context.agency_id IS NOT NULL THEN
    -- Agency mode: create all roles
  ...
  -- SELF/FAMILY mode: always create family and senior
  INSERT INTO user_profiles (id, role_id, display_name, agency_id, is_active)
  VALUES 
  ...
  -- Create departments (agency mode only)
  IF v_context.agency_id IS NOT NULL THEN
    ...
    -- Create task categories (agency mode only)
    ...
    -- Create observation events (agency mode only)
    FOR i IN 1..5 LOOP
    ...
    -- Create intelligence signals (agency mode only)
    ...
    -- Create risk scores (agency mode only)
  ...
  -- Create health metrics (all modes)
  ...
  -- Create health metric trends (all modes)
  ...
  VALUES
    ...
    (v_resident.id, 'LAB_WORK', 'Blood Work', 'Lab Services', now() + interval '14 days', 'SCHEDULED', 'Lab Center', 'Blood work')
  ON CONFLICT DO NOTHING;

  -- Create documents (all modes)
  ...
  VALUES
    (v_resident.id, 'DAILY', ARRAY['Low sodium'], ARRAY['Walker assistance'], 'Care plan established', COALESCE(v_admin_user_id, v_caregiver_user_id, v_family_user_id, v_senior_user_id), COALESCE(v_admin_user_id, v_caregiver_user_id, v_family_user_id, v_senior_user_id))
  ON CONFLICT DO NOTHING;

  -- Create device (all modes)
  ...
  -- Create notifications (all modes)
