/*
  # Fix Seed Function to Create Today's Tasks

  ## Problem
  WP1 test fails with "No unassigned tasks available" because:
  - Seed function creates tasks for day offsets 0-6 from base_time
  - Base time is NOW() - 3 days
  - Day offset 3 would be "today" but tasks are scheduled at specific times
  - If run at different times of day, tasks might not appear as "today's" tasks
  
  ## Solution
  Update seed function to explicitly create tasks for CURRENT_DATE at various times.
*/
...
BEGIN
  ...
  FOR v_resident_id IN SELECT unnest(v_resident_ids) LIMIT 10 LOOP
    ...
    INSERT INTO tasks (
      ...
      'KITCHEN_STAFF', true, v_system_user_id, NULL,
  ...
  FOR v_resident_id IN SELECT unnest(v_resident_ids) LIMIT 10 LOOP
    ...
    INSERT INTO tasks (
      ...
      'HOUSEKEEPER', true, v_system_user_id, NULL,
