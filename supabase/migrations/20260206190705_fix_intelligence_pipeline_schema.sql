/*
  # Fix Intelligence Pipeline Schema Match

  ## Purpose
  Update seed functions to match actual table schema:
  - intelligence_signals uses: category, title, description, reasoning
  - ai_learning_inputs uses: input_data instead of context
  - notification_log uses: metadata for custom fields
*/
...
BEGIN
  ...
  -- Step 3: Create intelligence signal (correct schema)
  ...
  -- Step 5: Create AI learning input (correct schema)
...
-- Verification function (correct schema)
...
'Seeds complete intelligence pipeline test data: vital anomaly → signal → notification → AI feedback (schema-corrected)';

COMMENT ON FUNCTION verify_intelligence_pipeline IS
'Verifies that intelligence pipeline is properly wired and all tables are populated (schema-corrected)';
