/*
  # Add More Deletions to Seed

  Adds deletion of conflict_test_scenarios and other referencing tables
*/
...
BEGIN
  ...
  -- Delete all test/showcase data for this agency
  ...
  -- Create health metrics (7 days of readings)
  FOR i IN 1..8 LOOP
    FOR j IN 0..6 LOOP
  ...
  -- Glucose metrics for diabetic resident (2 weeks)
  FOR j IN 0..13 LOOP
    INSERT INTO health_metric_trends (resident_id, metric_type, metric_value, metric_unit, recorded_at, data_source)
  ...
  VALUES
    (v_residents[1], 'CHRONIC_DISEASE', 'Diabetes & Hypertension Management', 'Comprehensive care plan for managing diabetes and high blood pressure', '["Maintain HbA1c below 7.0%", "Keep blood pressure below 140/90", "Improve medication adherence"]'::jsonb, '["Daily blood glucose monitoring", "Weekly blood pressure checks", "Medication adherence tracking", "Dietary consultation monthly"]'::jsonb, 'ACTIVE', v_today - interval '60 days', v_today + interval '30 days', 'Dr. Johnson'),
    (v_residents[2], 'CHRONIC_DISEASE', 'Cardiovascular Health', 'Post-cardiac event management and prevention plan', '["Maintain cholesterol levels within range", "Regular exercise program", "Cardiac rehabilitation completion"]'::jsonb, '["Monthly cardiology follow-up visits", "Daily low-dose aspirin", "Monitored exercise routine", "Heart-healthy diet"]'::jsonb, 'ACTIVE', v_today - interval '90 days', v_today + interval '60 days', 'Dr. Chen');
  ...
  VALUES
    (v_family_id, 'LAB_RESULT', 'IN_APP', 'New Lab Results Available', 'Lab results from January 30 blood work are now available for review in the Documents section', 'MEDIUM', 'DELIVERED', v_today - interval '5 days', '{"resident_name": "Dorothy Miller", "test_type": "Blood Work"}'::jsonb),
    (v_senior_id, 'MEDICATION_REMINDER', 'IN_APP', 'Medication Reminder', 'Time to take your morning medications: Lisinopril 10mg and Metformin 500mg', 'HIGH', 'DELIVERED', v_today + interval '9 hours', '{"medications": ["Lisinopril 10mg", "Metformin 500mg"]}'::jsonb);
  ...
  VALUES (v_agency_id, 'OPERATIONAL', 'ALL_CLEAR', 0, 0, '[]'::jsonb, now(), '{"residents_monitored": 8, "tasks_pending": 8, "staff_on_duty": 10}'::jsonb);
