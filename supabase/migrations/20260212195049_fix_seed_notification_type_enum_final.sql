/*
  # Fix notification_type enum values

  Changes:
  - Use 'CRITICAL', 'INFORMATIONAL' instead of 'health_alert', 'care_update', 'appointment_reminder'
*/
...
BEGIN
  ...
  VALUES
    (v_resident.id, COALESCE(v_family_user_id, v_senior_user_id, v_admin_user_id), 'CRITICAL', 'vital_sign', 'Recent BP reading shows elevation', ARRAY['IN_APP'], false, false, now() - interval '3 hours'),
    (v_resident.id, COALESCE(v_family_user_id, v_senior_user_id, v_admin_user_id), 'INFORMATIONAL', 'task_completion', 'Morning medication administered', ARRAY['IN_APP'], false, false, now() - interval '2 hours'),
    (v_resident.id, COALESCE(v_family_user_id, v_senior_user_id, v_admin_user_id), 'INFORMATIONAL', 'appointment', 'Checkup scheduled in 7 days', ARRAY['IN_APP'], false, false, now() - interval '1 hour')
  ON CONFLICT DO NOTHING;
