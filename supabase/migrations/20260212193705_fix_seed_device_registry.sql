/*
  # Fix device_registry schema match
*/
...
BEGIN
  ...
  IF v_context.service_model IN ('DIRECT_HIRE', 'AGENCY_HOME_CARE', 'AGENCY_FACILITY') THEN
    FOR i IN 1..10 LOOP INSERT INTO tasks (resident_id, category_id, title, description, priority, state, scheduled_for, assigned_to, department_id, risk_level) VALUES (v_resident.id, CASE WHEN i <= 4 THEN v_task_cat_med_id WHEN i <= 7 THEN v_task_cat_meal_id ELSE v_task_cat_hygiene_id END, CASE WHEN i <= 4 THEN 'Administer Medication' WHEN i <= 7 THEN 'Serve Meal' ELSE 'Assist with Hygiene' END, 'Routine care', CASE WHEN i <= 3 THEN 'high' WHEN i <= 7 THEN 'medium' ELSE 'low' END, CASE WHEN i <= 3 THEN 'due' WHEN i <= 5 THEN 'in_progress' ELSE 'completed' END, now() + (i || ' hours')::interval, v_caregiver_user_id, v_dept_nursing_id, CASE WHEN i <= 3 THEN 'B' ELSE 'C' END) ON CONFLICT DO NOTHING; END LOOP;
  END IF;
  INSERT INTO resident_medications (resident_id, medication_name, dosage, frequency, route, prescriber_name, is_active, start_date, entered_by) VALUES (v_resident.id, 'Lisinopril', '10 mg', 'Once daily', 'ORAL', 'Dr. Sarah Johnson', true, CURRENT_DATE, COALESCE(v_caregiver_user_id, v_admin_user_id, v_senior_user_id, v_family_user_id)), (v_resident.id, 'Metformin', '500 mg', 'Twice daily', 'ORAL', 'Dr. Sarah Johnson', true, CURRENT_DATE, COALESCE(v_caregiver_user_id, v_admin_user_id, v_senior_user_id, v_family_user_id)) ON CONFLICT DO NOTHING;
  FOR i IN 0..4 LOOP
    INSERT INTO vital_signs (resident_id, vital_type, value, recorded_at, recorded_by, systolic, diastolic) VALUES (v_resident.id, 'BLOOD_PRESSURE', (120 + i*2) || '/' || (80 + i), now() - (i || ' days')::interval, COALESCE(v_caregiver_user_id, v_senior_user_id), 120 + i*2, 80 + i), (v_resident.id, 'HEART_RATE', (72 + i)::text, now() - (i || ' days')::interval, COALESCE(v_caregiver_user_id, v_senior_user_id), null, null) ON CONFLICT DO NOTHING;
    INSERT INTO health_metrics (resident_id, metric_category, metric_type, value_numeric, unit, recorded_at, measurement_source, data_source, confidence_level) VALUES (v_resident.id, 'BLOOD_PRESSURE', 'blood_pressure_systolic', 120 + i*2, 'mmHg', now() - (i || ' days')::interval, 'MANUAL_ENTRY', 'MANUAL_ENTRY', 'MEDIUM'), (v_resident.id, 'CARDIOVASCULAR', 'heart_rate', 72 + i, 'bpm', now() - (i || ' days')::interval, 'MANUAL_ENTRY', 'MANUAL_ENTRY', 'MEDIUM') ON CONFLICT DO NOTHING;
  END LOOP;
  INSERT INTO health_metric_trends (resident_id, metric_type, period, avg_value, min_value, max_value, sample_count, last_calculated_at, trend_direction) VALUES (v_resident.id, 'blood_pressure_systolic', 'DAY_7', 122, 120, 128, 5, now(), 'STABLE'), (v_resident.id, 'heart_rate', 'DAY_7', 74, 72, 76, 5, now(), 'INCREASING') ON CONFLICT DO NOTHING;
  INSERT INTO appointments (resident_id, appointment_type, title, scheduled_at, duration_minutes, status, provider_name, location) VALUES (v_resident.id, 'DOCTOR_VISIT', 'Annual Physical', now() + interval '7 days', 60, 'SCHEDULED', 'Dr. Sarah Johnson', 'Main Clinic'), (v_resident.id, 'SCREENING', 'Blood Work', now() + interval '14 days', 30, 'SCHEDULED', 'LabCorp', 'Lab Center') ON CONFLICT DO NOTHING;
  INSERT INTO resident_documents (resident_id, title, file_name, storage_path, uploaded_by) VALUES (v_resident.id, 'Care Plan', 'care-plan.pdf', '/documents/care-plan.pdf', COALESCE(v_admin_user_id, v_senior_user_id)), (v_resident.id, 'Medical History', 'medical-history.pdf', '/documents/medical-history.pdf', COALESCE(v_admin_user_id, v_senior_user_id)) ON CONFLICT DO NOTHING;
  INSERT INTO resident_care_plan_anchors (resident_id, care_frequency, dietary_restrictions, mobility_assistance_needs, entered_by, language_context) VALUES (v_resident.id, 'DAILY', ARRAY['Low sodium'], ARRAY['Minimal assistance'], COALESCE(v_admin_user_id, v_caregiver_user_id, v_senior_user_id), 'Care plan established') ON CONFLICT DO NOTHING;
  INSERT INTO device_registry (device_id, resident_id, device_type, device_name, manufacturer, model, firmware_version, trust_state, pairing_actor) VALUES ('DEV-' || substring(v_resident.id::text, 1, 8), v_resident.id, 'BLE_HEALTH_SENSOR', 'BP Monitor', 'Omron', 'BP7000', '1.0.0', 'TRUSTED', COALESCE(v_senior_user_id, v_caregiver_user_id, v_admin_user_id)) ON CONFLICT DO NOTHING;
  FOR i IN 1..5 LOOP INSERT INTO observation_events (resident_id, event_type, event_data, quality_score, captured_at, captured_by) VALUES (v_resident.id, 'task_completion', jsonb_build_object('task_type', 'medication', 'completed', true), 80 + i, now() - (i || ' hours')::interval, COALESCE(v_caregiver_user_id, v_senior_user_id)) ON CONFLICT DO NOTHING; END LOOP;
  INSERT INTO intelligence_signals (resident_id, signal_type, signal_category, category, severity, title, description, detected_at) VALUES (v_resident.id, 'PATTERN', 'MEDICATION_ADHERENCE', 'REACTIVE', 'LOW', 'High Medication Adherence', 'Patient consistently taking medications on time', now()), (v_resident.id, 'ANOMALY', 'VITAL_TREND', 'PREDICTIVE', 'MEDIUM', 'Blood Pressure Trending Up', 'Systolic BP increased over 5 days', now()) ON CONFLICT DO NOTHING;
