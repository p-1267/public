/*
  # Fix Seed Function Schema Mismatches

  Updates seed_active_context to match actual table schemas
*/
...
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'Resident % not found', v_context.resident_id; END IF;
  ...
  IF v_context.care_setting = 'FACILITY' OR v_context.service_model = 'AGENCY_FACILITY' THEN
    ...
    VALUES (gen_random_uuid(), v_agency_id, 'Nursing', 'NURSING', true),
           ...
           (gen_random_uuid(), v_agency_id, 'Housekeeping', 'HOUSEKEEPING', true)
  ...
  VALUES (v_resident.id, 'CARE_PLAN', 'Care Plan', v_admin_user_id, '/documents/care-plan.pdf'),
         (v_resident.id, 'MEDICAL_RECORD', 'Medical History', v_admin_user_id, '/documents/medical-history.pdf')
  ON CONFLICT DO NOTHING;

  INSERT INTO resident_care_plan_anchors (resident_id, plan_status, primary_diagnosis, care_goals)
  VALUES (v_resident.id, 'ACTIVE', 'Hypertension, Type 2 Diabetes', 'Maintain stable vitals, medication adherence')
  ON CONFLICT DO NOTHING;

  INSERT INTO device_registry (resident_id, device_type, device_name, manufacturer, model, paired_at, is_active)
  VALUES (v_resident.id, 'BLOOD_PRESSURE_MONITOR', 'BP Monitor', 'Omron', 'BP7000', now(), true)
  ON CONFLICT DO NOTHING;

  FOR i IN 1..5 LOOP
    INSERT INTO observation_events (resident_id, event_type, event_data, quality_score, captured_at, captured_by)
    VALUES (v_resident.id, 'TASK_COMPLETION', jsonb_build_object('task_type', 'medication', 'completed', true), 80 + i, now() - (i || ' hours')::interval, v_caregiver_user_id)
    ON CONFLICT DO NOTHING;
  END LOOP;
  ...
  VALUES (v_resident.id, 'PATTERN', 'MEDICATION_ADHERENCE', 'INFO', 'High Medication Adherence', 'Patient consistently taking medications on time', now()),
         (v_resident.id, 'ANOMALY', 'VITAL_TREND', 'MEDIUM', 'Blood Pressure Trending Up', 'Systolic BP increased by 8 points over 5 days', now())
  ON CONFLICT DO NOTHING;
  ...
  IF v_senior_user_id IS NOT NULL THEN
    FOR i IN 1..3 LOOP
      ...
      VALUES (v_senior_user_id, v_resident.id, 'MEDICATION_REMINDER', 'Medication Due', 'Time to take Lisinopril', now() - (i || ' hours')::interval)
      ON CONFLICT DO NOTHING;
    END LOOP;
  ...
  IF v_family_user_id IS NOT NULL THEN
    FOR i IN 1..3 LOOP
      INSERT INTO notification_log (user_id, resident_id, notification_type, title, message, sent_at)
      VALUES (v_family_user_id, v_resident.id, 'HEALTH_UPDATE', 'Health Status Update', 'Blood pressure reading recorded', now() - (i || ' hours')::interval)
      ON CONFLICT DO NOTHING;
    END LOOP;
  END IF;
  ...
  RETURN jsonb_build_object('status', 'SUCCESS', 'care_context_id', p_care_context_id, 'resident_id', v_resident.id,
    'management_mode', v_context.management_mode, 'service_model', v_context.service_model);
END;
$$;
