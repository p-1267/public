/*
  # Fix seed_active_context entered_by NULL constraint

  Problem: entered_by is NOT NULL but COALESCE only checks 3 user types
  Solution: Expand COALESCE to include senior_user_id and family_user_id
*/
...
BEGIN
  ...
  IF v_entered_by IS NULL THEN
    RAISE EXCEPTION 'No valid user_id available for entered_by in scenario %', v_context.management_mode;
  END IF;
  ...
  FOR i IN 0..4 LOOP
    ...
    VALUES
      (v_resident.id, 'BLOOD_PRESSURE_SYSTOLIC', 120 + (i * 2), 'mmHg', now() - (i || ' days')::interval, v_entered_by, 'MANUAL'),
      (v_resident.id, 'HEART_RATE', 72 + i, 'bpm', now() - (i || ' days')::interval, v_entered_by, 'MANUAL')
    ON CONFLICT DO NOTHING;
