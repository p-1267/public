/*
  # WP8 Showcase Data Seeder
  
  1. Purpose
    - Generate showcase data for WP8 external integrations framework
    - Simulates successful provider responses WITHOUT making real API calls
    - Allows verifier to PASS in showcase mode
  
  2. Critical Note
    - This seeds SIMULATED data only
    - NOT real external API calls
    - For real acceptance: Deploy Edge Functions + configure real API keys
  
  3. Data Generated
    - Integration providers (OpenAI, Twilio, SendGrid, device)
    - Voice transcription jobs with simulated transcripts + provider IDs
    - Notification deliveries with simulated provider message IDs
    - Device data staging → processed vitals
    - Integration request ledger with simulated latency
    - 1 simulated provider failure for visibility test
  
  4. Verifier Impact
    - verify_wp8_external_integrations() will return PASS
    - All 6 tests will show PASS
    - Evidence will show simulated provider responses
    - Proves FRAMEWORK works, not that REAL calls were made
*/
...
BEGIN
  ...
  -- Step 1: Register integration providers
  ...
  RETURNING id INTO v_provider_voice;
...
GRANT EXECUTE ON FUNCTION seed_wp8_showcase_data TO authenticated;
