/*
  # Fix Correlation Engine - Use Outcome Column

  1. Issue
    - tasks doesn't have notes column
    - Has outcome column with possible CONCERN value

  2. Fix
    - Use outcome = 'CONCERN' instead of notes search
*/
    ...
    IF v_rule.rule_name = 'medication_adherence_vitals_pattern' THEN
      IF v_med_late_count >= 2 AND v_vitals_abnormal_count >= 1 THEN
        ...
        INSERT INTO compound_intelligence_events (
          ...
          0.85, v_reasoning_text, v_reasoning_details,
          v_time_start, v_time_end, v_med_late_count + v_vitals_abnormal_count, v_rule.requires_human_action
    ...
    IF v_rule.rule_name = 'family_concern_caregiver_observation' THEN
      IF v_family_urgent_count >= 1 AND v_task_concern_count >= 1 THEN
        ...
        SELECT v_compound_event_id, 'tasks', id, 'task_completion', actual_end,
          jsonb_build_object('state', state, 'task_name', task_name, 'outcome', outcome)
        FROM tasks
        WHERE resident_id = p_resident_id AND actual_end >= v_time_start AND outcome = 'CONCERN'
        LIMIT 5;
