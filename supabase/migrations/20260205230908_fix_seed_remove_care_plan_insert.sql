/*
  # Fix Seed Function - Remove incompatible care plan insert

  The resident_care_plan_anchors table has a different schema than expected.
  Remove the insert that uses care_focus_area, goals, interventions, review_frequency
  since those columns don't exist.
*/
...
BEGIN
  ...
  RETURN jsonb_build_object('status', 'SUCCESS', 'message', 'Senior + Family scenario seeded successfully', 'resident_id', v_resident_id, 'functional_pages', '22 of 22', 'data', jsonb_build_object('medications', 2, 'appointments', 3, 'lab_tests', 3, 'documents', 3, 'emergency_contacts', 3, 'notifications', 5, 'vital_signs', v_vital_count, 'health_metrics', (SELECT COUNT(*) FROM health_metrics WHERE resident_id = v_resident_id), 'health_trends', (SELECT COUNT(*) FROM health_metric_trends WHERE resident_id = v_resident_id)));
END;
$$;