/*
  # STEP 5: Updated verifier to use voice_transcriptions table

  Creates test data in voice_transcriptions (not voice_transcription_jobs)
*/
...
BEGIN
  ...
  BEGIN
    ...
    SELECT (classify_voice_intent(v_test_transcription_id, p_agency_id, 'Patient ate full breakfast, appeared in good mood', 'en', v_test_resident_id, v_test_user_id)->>'intent_id')::uuid
    INTO v_intent_id;
    
    SELECT (generate_action_draft_from_intent(v_intent_id, p_agency_id, v_test_resident_id, v_test_user_id)->>'draft_id')::uuid
    INTO v_draft_id;
    
    v_commit_result := confirm_and_commit_voice_action(v_draft_id, v_test_user_id, 'approve');
    
    IF v_commit_result->>'observation_event_id' IS NOT NULL AND v_commit_result->>'audit_log_id' IS NOT NULL THEN
      v_check_result := 'PASS';
    ELSE
      v_check_result := 'FAIL';
  ...
  BEGIN
    ...
    SELECT (classify_voice_intent(v_test_transcription_id, p_agency_id, 'Need more supplies in room 201', 'en', v_test_resident_id, v_test_user_id)->>'intent_id')::uuid
    INTO v_intent_id;
    ...
    IF v_commit_result->>'task_id' IS NOT NULL AND v_commit_result->>'audit_log_id' IS NOT NULL THEN
      v_check_result := 'PASS';
    ELSE
      v_check_result := 'FAIL';
      v_overall_status := 'FAIL';
  ...
  BEGIN
    ...
    SELECT (classify_voice_intent(v_test_transcription_id, p_agency_id, 'Patient fell in bathroom, help needed urgently', 'en', v_test_resident_id, v_test_user_id)->>'intent_id')::uuid
    INTO v_intent_id;
  ...
  BEGIN
    ...
    SELECT (classify_voice_intent(v_test_transcription_id, p_agency_id, 'Schedule doctor appointment for tomorrow', 'en', v_test_resident_id, v_test_user_id)->>'intent_id')::uuid
    INTO v_intent_id;
    ...
    IF v_commit_result->>'appointment_id' IS NOT NULL AND v_commit_result->>'audit_log_id' IS NOT NULL THEN
      v_check_result := 'PASS';
    ELSE
      v_check_result := 'FAIL';
      v_overall_status := 'FAIL';
  ...
  BEGIN
    ...
    SELECT (classify_voice_intent(v_test_transcription_id, p_agency_id, 'Give insulin dose now', 'en', v_test_resident_id, v_test_user_id)->>'intent_id')::uuid
    INTO v_intent_id;
    ...
    IF v_commit_result->>'medication_log_id' IS NOT NULL AND v_commit_result->>'audit_log_id' IS NOT NULL THEN
      v_check_result := 'PASS';
    ELSE
      v_check_result := 'FAIL';
      v_overall_status := 'FAIL';
