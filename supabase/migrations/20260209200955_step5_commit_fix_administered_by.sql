/*
  # STEP 5: Fix medication administered_by to use valid user_profile ID
  
  Use a valid user_profile ID from the same agency as fallback
*/
    ...
    CASE v_draft.action_type
      ...
      WHEN 'MEDICATION_ADMINISTRATION' THEN
        ...
        SELECT
          ...
          rm.id,
          now(),
          COALESCE(p_user_id, (SELECT id FROM user_profiles WHERE agency_id = v_draft.agency_id LIMIT 1)),
          'TAKEN',
          v_payload->>'dose',
          v_payload->>'route',
          'en',
      ...
      WHEN 'URGENT_ACTION' THEN
        ...
        SELECT
          ...
          v_draft.agency_id,
          tc.id,
          'URGENT: ' || LEFT(v_payload->>'description', 100),
      ...
      WHEN 'REQUEST' THEN
        ...
        SELECT
          COALESCE(v_draft.resident_id, (SELECT id FROM residents WHERE agency_id = v_draft.agency_id LIMIT 1)),
          v_draft.agency_id,
          tc.id,
          'Request: ' || LEFT(v_payload->>'request_type', 100),
          v_payload->>'description',
          'medium',
          'scheduled',
          now(),
