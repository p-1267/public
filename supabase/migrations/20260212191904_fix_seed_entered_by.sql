/*
  # Fix Seed Function - Add entered_by
  
  Add entered_by to resident_medications insert
*/
...
BEGIN
  ...
  END IF;
  INSERT INTO resident_medications (resident_id, medication_name, dosage, frequency, route, prescriber_name, is_active, start_date, entered_by) VALUES (v_resident.id, 'Lisinopril', '10 mg', 'Once daily', 'ORAL', 'Dr. Sarah Johnson', true, CURRENT_DATE, COALESCE(v_caregiver_user_id, v_admin_user_id)), (v_resident.id, 'Metformin', '500 mg', 'Twice daily', 'ORAL', 'Dr. Sarah Johnson', true, CURRENT_DATE, COALESCE(v_caregiver_user_id, v_admin_user_id)) ON CONFLICT DO NOTHING;
  FOR i IN 0..4 LOOP
