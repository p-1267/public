-- Add caregiver tasks to seed function for 18/18 PASS
...
BEGIN
  ...
  -- Delete existing tasks for this resident
  DELETE FROM tasks WHERE resident_id = v_resident_id;

  -- NOW: 3 tasks IN_PROGRESS (currently doing)
  ...
  VALUES
    (v_agency_id, v_resident_id, v_cat_medication, 'Administer 8AM Medications', 'Lisinopril 10mg, Metformin 500mg', 'high', 'MEDIUM', 'IN_PROGRESS', CURRENT_TIMESTAMP - interval '10 minutes', CURRENT_TIMESTAMP + interval '20 minutes', CURRENT_TIMESTAMP - interval '10 minutes', v_caregiver_user_id, true, false, false, 0, false, false),
    (v_agency_id, v_resident_id, v_cat_vitals, 'Morning Vital Signs', 'BP, HR, Temp, O2 sat', 'high', 'LOW', 'IN_PROGRESS', CURRENT_TIMESTAMP - interval '15 minutes', CURRENT_TIMESTAMP + interval '15 minutes', CURRENT_TIMESTAMP - interval '15 minutes', v_caregiver_user_id, true, false, false, 0, false, false),
    (v_agency_id, v_resident_id, v_cat_wellness, 'Wellness Check-in', 'Assess mood, pain level, sleep quality', 'medium', 'LOW', 'IN_PROGRESS', CURRENT_TIMESTAMP - interval '5 minutes', CURRENT_TIMESTAMP + interval '25 minutes', CURRENT_TIMESTAMP - interval '5 minutes', v_caregiver_user_id, false, false, false, 0, false, false);

  -- NOW: 2 tasks PENDING (up next in current shift)
  ...
  VALUES
    (v_agency_id, v_resident_id, v_cat_clinical, 'Wound Care - Left Leg', 'Clean and redress surgical site', 'high', 'MEDIUM', 'PENDING', CURRENT_TIMESTAMP + interval '30 minutes', CURRENT_TIMESTAMP + interval '50 minutes', v_caregiver_user_id, true, false, false, 0, false, false),
    (v_agency_id, v_resident_id, v_cat_wellness, 'Hydration Check', 'Ensure adequate fluid intake', 'medium', 'LOW', 'PENDING', CURRENT_TIMESTAMP + interval '1 hour', CURRENT_TIMESTAMP + interval '1 hour 15 minutes', v_caregiver_user_id, false, false, false, 0, false, false);

  -- NEXT: 2 tasks (upcoming in next 2-4 hours)
  INSERT INTO tasks (agency_id, resident_id, category_id, task_name, description, priority, risk_level, state, scheduled_start, scheduled_end, owner_user_id, requires_evidence, evidence_submitted, is_recurring, escalation_level, is_emergency, is_blocked)
  VALUES
    (v_agency_id, v_resident_id, v_cat_medication, 'Administer 12PM Medications', 'As prescribed', 'high', 'MEDIUM', 'PENDING', CURRENT_TIMESTAMP + interval '3 hours', CURRENT_TIMESTAMP + interval '3 hours 30 minutes', v_caregiver_user_id, true, false, false, 0, false, false),
    (v_agency_id, v_resident_id, v_cat_vitals, 'Afternoon Vital Check', 'BP, HR per care plan', 'medium', 'LOW', 'PENDING', CURRENT_TIMESTAMP + interval '3 hours 30 minutes', CURRENT_TIMESTAMP + interval '3 hours 45 minutes', v_caregiver_user_id, true, false, false, 0, false, false);

  -- LATER: 3 tasks (rest of day / future shift)
  INSERT INTO tasks (agency_id, resident_id, category_id, task_name, description, priority, risk_level, state, scheduled_start, scheduled_end, owner_user_id, requires_evidence, evidence_submitted, is_recurring, escalation_level, is_emergency, is_blocked)
  VALUES
    (v_agency_id, v_resident_id, v_cat_medication, 'Administer 6PM Medications', 'Evening dose Metformin 500mg', 'high', 'MEDIUM', 'PENDING', CURRENT_TIMESTAMP + interval '9 hours', CURRENT_TIMESTAMP + interval '9 hours 30 minutes', v_caregiver_user_id, true, false, false, 0, false, false),
    (v_agency_id, v_resident_id, v_cat_vitals, 'Evening Vital Check', 'Final vitals before bed', 'medium', 'LOW', 'PENDING', CURRENT_TIMESTAMP + interval '10 hours', CURRENT_TIMESTAMP + interval '10 hours 15 minutes', v_caregiver_user_id, true, false, false, 0, false, false),
    (v_agency_id, v_resident_id, v_cat_wellness, 'Bedtime Routine', 'Prepare for sleep, ensure comfort', 'low', 'LOW', 'PENDING', CURRENT_TIMESTAMP + interval '12 hours', CURRENT_TIMESTAMP + interval '12 hours 30 minutes', v_caregiver_user_id, false, false, false, 0, false, false);
