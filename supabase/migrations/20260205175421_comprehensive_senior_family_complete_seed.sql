/*
  # Comprehensive Senior/Family Complete Seed
  
  Populates ALL tables required for Senior + Family pages to render:
  - residents, medications, appointments (existing)
  - vital_signs (FIXED: full 72 hours)
  - lab_tests (NEW)
  - resident_documents (NEW)
  - resident_care_plan_anchors (NEW)
  - resident_emergency_contacts (NEW)
  - notification_log (NEW)
  - message_threads + messages (NEW)
*/
...
BEGIN
  ...
  -- Resident Documents (3)
  INSERT INTO resident_documents (
    resident_id, title, description
  )
  VALUES
    (v_resident_id, 'Annual Physical Results 2025', 
     'Complete results from annual physical examination including vitals, lab work, and physician notes'),
    (v_resident_id, 'Medicare Card - Front and Back', 
     'Scanned copies of Medicare Part B insurance card'),
    (v_resident_id, 'Advanced Healthcare Directive', 
     'Legal document outlining healthcare wishes and power of attorney');
  ...
  VALUES
    (v_resident_id, 'DAILY', ARRAY['HANDRAILS', 'NON_SLIP_FOOTWEAR'], 
     ...
     'Prefers morning activities. Monitors BP and glucose at home.', v_family_user_id),
    (v_resident_id, 'WEEKLY', ARRAY[]::text[], 
     '{}'::jsonb, 'ENGLISH', ARRAY[]::text[], ARRAY[]::text[], ARRAY[]::text[],
     'Medication management - takes Lisinopril and Metformin daily', v_family_user_id);
  ...
  VALUES
    (v_resident_id, v_senior_user_id, 'MEDICATION_REMINDER', 'INFO',
     'Time for morning medications: Lisinopril 10mg and Metformin 500mg',
     ARRAY['PUSH'], false, false, now() - interval '2 hours'),
    (v_resident_id, v_family_user_id, 'APPOINTMENT_REMINDER', 'INFO',
    ...
    (v_resident_id, v_senior_user_id, 'HEALTH_ALERT', 'MEDIUM',
  ...
  -- Vital Signs (FIXED: Full 72 hours of readings)
  FOR i IN 0..71 LOOP
    v_timestamp := now() - (i || ' hours')::interval;
    
    -- BP readings 3x per day: 8 AM, 2 PM, 8 PM
    IF extract(hour from v_timestamp) = 8 THEN
      ...
      INSERT INTO vital_signs (
        ...
        v_systolic, v_diastolic, v_timestamp, v_senior_user_id, 'Morning reading'
      );
    ...
    ELSIF extract(hour from v_timestamp) = 14 THEN
      ...
      INSERT INTO vital_signs (
        ...
        v_resident_id, 'BLOOD_PRESSURE', v_systolic || '/' || v_diastolic,
        v_systolic, v_diastolic, v_timestamp, v_senior_user_id, 'Afternoon reading'
      );
      v_vital_count := v_vital_count + 1;
    ELSIF extract(hour from v_timestamp) = 20 THEN
      ...
      INSERT INTO vital_signs (
        ...
        v_resident_id, 'BLOOD_PRESSURE', v_systolic || '/' || v_diastolic,
        v_systolic, v_diastolic, v_timestamp, v_senior_user_id, 'Evening reading'
      );
      v_vital_count := v_vital_count + 1;
    END IF;
    
    -- Glucose 1x per day: 7 AM
    IF extract(hour from v_timestamp) = 7 THEN
      INSERT INTO vital_signs (
        ...
        v_timestamp, v_senior_user_id, 'Fasting glucose'
      );
      v_vital_count := v_vital_count + 1;
    END IF;
  END LOOP;
  ...
  RETURN jsonb_build_object(
    'status', 'SUCCESS',
    'message', 'Senior + Family scenario FULLY seeded with ALL required data',
