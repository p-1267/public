/*
  # Billing Exports Table (Phase 25)

  ## Purpose
  Immutable billing export records for insurance claims.
  ...
  - `billing_exports`
    ...
    - `total_units` (numeric) - total billable units
...
CREATE INDEX IF NOT EXISTS idx_billing_exports_agency_id ON billing_exports(agency_id);
CREATE INDEX IF NOT EXISTS idx_billing_exports_date_range ON billing_exports(start_date, end_date);
CREATE INDEX IF NOT EXISTS idx_billing_exports_is_sealed ON billing_exports(is_sealed);
CREATE INDEX IF NOT EXISTS idx_billing_exports_generated_at ON billing_exports(generated_at DESC);
