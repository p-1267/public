/*
  # Intelligence Pipeline Uses Production RPCs

  ## Purpose
  Update intelligence pipeline test to use PRODUCTION RPCs only.
  No test-specific inserts - uses same code paths as production.

  ## Flow
  1. record_vital_sign_with_simulation_tag() - Creates vitals
  2. create_intelligence_signal() - Brain detects anomaly
  3. TRIGGER automatically creates notification
  4. submit_ai_feedback() - User provides feedback

  CRITICAL: All RPCs are production code, not test helpers
*/
...
BEGIN
  ...
  -- This triggers anomaly detection
  -- ============================================================
  ...
  -- ============================================================
  -- STEP 3: Brain detects anomaly and creates signal using PRODUCTION RPC
  -- ============================================================
  SELECT create_intelligence_signal(
    'BP_SPIKE_' || extract(epoch from now())::bigint,  -- signal_id
    'PREDICTIVE',  -- category
    'HIGH',  -- severity (triggers notification automatically)
    v_resident_id,  -- resident_id
    v_agency_id,  -- agency_id
    'Blood Pressure Significantly Elevated',  -- title
    'Reading of 165/95 is 37.5% above baseline of 120/80',  -- description
    'Systolic BP increased from average 120 to 165. This represents a significant deviation. Stress reported by resident may be contributing factor.',  -- reasoning
    ARRAY['Recheck BP in 15 minutes', 'Contact primary physician if sustained', 'Review recent medication adherence'],  -- suggested_actions
    ARRAY['vital_signs', 'resident_baselines'],  -- data_source
    true  -- is_simulation
  ) INTO v_signal_result;

  -- Wait briefly to ensure trigger has fired
