/*
  # Create Universal Seed Function

  Creates comprehensive seed_active_context RPC that:
  - Seeds ALL features per SEED_SCOPE_TRUTH.md
  - Context-driven (management_mode, care_setting, service_model)
  - Idempotent and deterministic
  - Populates intelligence pipeline inputs
*/
...
BEGIN
  ...
  IF v_context.care_setting = 'FACILITY' OR v_context.service_model = 'AGENCY_FACILITY' THEN
    ...
    VALUES
      (gen_random_uuid(), v_agency_id, 'Nursing', 'NURSING', true),
  ...
  VALUES
    (v_resident.id, 'CARE_PLAN', 'Care Plan', v_admin_user_id, '/documents/care-plan.pdf'),
  ...
  VALUES
    (v_resident.id, 'PATTERN', 'MEDICATION_ADHERENCE', 'INFO', 'High Medication Adherence', 'Patient consistently taking medications on time', now()),
  ...
  VALUES (v_resident.id, 'FALL_RISK', 'MEDIUM', 45, now())
  ON CONFLICT DO NOTHING;
