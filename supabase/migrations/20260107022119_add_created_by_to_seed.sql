/*
  # Add created_by to Seed Tasks
  
  The tasks table has a NOT NULL constraint on created_by. This adds it to all task inserts.
*/
...
BEGIN
  ...
  INSERT INTO task_templates (agency_id, category_id, template_name, description, default_duration_minutes, default_priority, is_active, created_at, created_by)
  VALUES
    (v_agency_id, v_category_nursing, 'Morning Medication', 'Administer morning medications', 15, 'high', true, now(), v_supervisor_id),
    (v_agency_id, v_category_nursing, 'Vital Signs Check', 'Monitor and record vital signs', 10, 'medium', true, now(), v_supervisor_id),
    (v_agency_id, v_category_housekeeping, 'Room Cleaning', 'Complete room cleaning and sanitization', 30, 'medium', true, now(), v_supervisor_id),
    (v_agency_id, v_category_kitchen, 'Breakfast Delivery', 'Deliver and serve breakfast', 10, 'high', true, now(), v_supervisor_id),
    (v_agency_id, v_category_hygiene, 'Morning Hygiene', 'Assist with morning personal care', 25, 'high', true, now(), v_supervisor_id)
  ...
  FOR i IN 1..2 LOOP
    ...
    INSERT INTO tasks (
      ...
      is_emergency, is_blocked, created_by, created_at, updated_at
    ) VALUES (
      ...
      true, false, false, false, false, v_supervisor_id, now(), now()
    );
    ...
    INSERT INTO tasks (
      ...
      category_id, template_id, owner_user_id, responsibility_role,
      requires_evidence, evidence_submitted, supervisor_acknowledged,
      actual_start, actual_end, outcome, completed_by,
      is_emergency, is_blocked, created_by, created_at, updated_at
    ) VALUES (
      ...
      false, false, true,
      v_today + interval '7 hours 28 minutes', v_today + interval '7 hours 38 minutes', 'success', v_caregiver_id,
      false, false, v_supervisor_id, now(), now()
    );
    ...
    INSERT INTO tasks (
      ...
      false, false, false, false, false, v_supervisor_id, now(), now()
    );
    ...
    INSERT INTO tasks (
      ...
      true, true, false,
      v_today + interval '6 hours 58 minutes', v_today + interval '7 hours 23 minutes', 'success', v_caregiver_id,
      false, false, v_supervisor_id, now(), now()
    );
    
    -- Lunch (12:00 PM - scheduled)
    INSERT INTO tasks (
      agency_id, resident_id, task_name, department, priority,
      scheduled_start, scheduled_end, duration_minutes, state,
      category_id, template_id, owner_user_id, responsibility_role,
  ...
  INSERT INTO brain_state (
    ...
    (v_resident_ids[2], 'stable', 'stable',
     'All indicators normal, positive trends observed', 0.95, 'A',
     0, '[]'::jsonb, '[]'::jsonb,
     now(), '{"assessment_trigger": "routine", "positive_signals": 1}'::jsonb, 1,
     now(), now());
