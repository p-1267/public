/*
  # Fix seed_showcase_scenario to Include Departments
  
  Updates the seed_showcase_scenario RPC to create department data alongside
  residents, tasks, and categories. This ensures departments are always available
  when showcase mode is initialized.
  
  Changes:
  - Adds department creation after agency creation
  - Creates 7 departments: Nursing, Housekeeping, Kitchen, Hygiene, Mobility, Nutrition, Monitoring
  - Adds department_personnel records for each department
...
BEGIN
  ...
  -- Create department personnel - Nursing (12 staff)
  ...
  SELECT 
    ...
    true,
    ARRAY['cleaning', 'sanitization'],
    CASE WHEN i % 3 = 0 THEN 'on_shift' ELSE 'off_shift' END,
    '555-02' || LPAD(i::text, 2, '0'),
    'housekeeper' || i::text || '@demo.com',
    (i % 4) + 1
  FROM generate_series(1, 8) AS i;
  
  -- Create department personnel - Kitchen (6 staff)
  INSERT INTO department_personnel (agency_id, department_id, user_id, first_name, last_name, display_name, employee_id, position_title, shift_pattern, is_primary_department, skills, status, work_phone, work_email, workload_indicator)
  SELECT 
    v_agency_id,
    v_dept_kitchen,
  ...
  -- Create tasks (keeping existing logic)
  ...
  v_result := jsonb_build_object(
    ...
    'message', 'Showcase scenario seeded successfully with departments and personnel'
  );
