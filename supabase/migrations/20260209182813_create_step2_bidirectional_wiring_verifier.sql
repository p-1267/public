/*
  # Step 2 Bidirectional Wiring Verifier

  1. Purpose
    - Verify family can submit observations (RLS enforced)
    - Verify audit logging with actor attribution
    - Verify routing to supervisor exception queue
    - Verify supervisor can route to caregiver
    - Verify unified timeline shows all actors
    - Verify brain pipeline consumes family input

  2. Tests (8 checks)
    1. Family observation submission creates record
    2. Audit log entry created with correct actor
    3. Supervisor exception queue entry created
    4. Unified timeline entry created
    5. Observation_event created for brain
    6. Supervisor can process observation
    7. Caregiver completion updates timeline
    8. Brain pipeline processes observation_event
...
BEGIN
  ...
  FROM audit_log
  WHERE table_name = 'family_observations'
    AND record_id = v_observation_id
    AND action = 'INSERT';
...
GRANT EXECUTE ON FUNCTION verify_step2_bidirectional_wiring TO authenticated, anon;
