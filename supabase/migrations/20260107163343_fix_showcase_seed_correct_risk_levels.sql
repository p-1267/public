/*
  # Fix Showcase Seed - Correct Risk Levels (A/B/C)
*/
...
BEGIN
  ...
  FOR v_day_offset IN 0..6 LOOP
    -- Morning medications (8 AM) - Risk level B
    ...
    LOOP
      INSERT INTO tasks (
        ...
        jsonb_build_object(
          ...
          'completed_by_name', CASE WHEN v_day_offset < 3 THEN 'Sarah Williams' ELSE NULL END,
          'completed_by_role', 'RN'
        )
      );
    END LOOP;
    
    -- Breakfast delivery (8:30 AM) - Risk level C
    FOR v_resident_id IN SELECT unnest(v_resident_ids) LIMIT 20 LOOP
      INSERT INTO tasks (
        ...
        jsonb_build_object(
          ...
          'completed_by_name', CASE WHEN v_day_offset < 3 THEN 'Tom Harris' ELSE NULL END,
          'completed_by_role', 'Kitchen Staff'
        )
      );
    END LOOP;
    
    -- Lunch delivery (12:30 PM) - Risk level C
    FOR v_resident_id IN SELECT unnest(v_resident_ids) LIMIT 20 LOOP
      INSERT INTO tasks (
        agency_id, resident_id, category_id, department,
        task_name, description, priority, risk_level, state,
        ...
        jsonb_build_object(
          'meal_type', 'lunch',
          'diet_type', 'standard',
          'intake_percentage', CASE WHEN v_day_offset < 3 THEN 75 ELSE NULL END,
          'completed_by_name', CASE WHEN v_day_offset < 3 THEN 'Tom Harris' ELSE NULL END
        )
      );
    END LOOP;
    
    -- Room cleaning (10 AM) - Risk level C
    FOR v_resident_id IN 
    ...
    LOOP
      INSERT INTO tasks (
        ...
        CASE WHEN v_day_offset < 3 THEN true ELSE false END,
        jsonb_build_object(
          'tasks_completed', jsonb_build_array('swept_floor', 'sanitized_bathroom', 'changed_linens'),
          'voice_report', 'Room clean, no issues found',
          'completed_by_name', CASE WHEN v_day_offset < 3 THEN 'Maria Gonzalez' ELSE NULL END,
          'completed_by_role', 'Housekeeping Staff',
          'supervisor_name', 'Carlos Rodriguez'
        )
      );
    END LOOP;
    
    -- Vital signs monitoring (2 PM) - Risk level B
    FOR v_resident_id IN 
      SELECT id FROM residents 
      WHERE agency_id = v_agency_id 
      AND metadata->>'nursing_acuity' = 'high'
      LIMIT 7
    LOOP
      INSERT INTO tasks (
        agency_id, resident_id, category_id, department,
        ...
        v_agency_id, v_resident_id, v_cat_nursing_vitals, 'NURSING',
        'Afternoon Vital Signs Check', 'Monitor BP, HR, Temp, O2',
        'medium', 'B',
        CASE WHEN v_day_offset < 3 THEN 'completed' ELSE 'pending' END,
        ...
        jsonb_build_object(
          ...
          'completed_by_name', CASE WHEN v_day_offset < 3 THEN 'Robert Chen' ELSE NULL END,
          'completed_by_role', 'CNA'
        )
      );
    END LOOP;
