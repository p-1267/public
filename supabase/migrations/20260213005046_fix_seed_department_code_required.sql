/*
  # Fix seed_active_context to include department_code

  ## Change
  - Add department_code to departments INSERT statement
  - Use deterministic code 'PERSCARE' for 'Personal Care' department

  ## Fixes
  - ERROR: null value in column "department_code" violates not-null constraint
  - Affects Hybrid and Agency scenarios that create departments
*/
...
BEGIN
  ...
  IF v_context.service_model IN ('DIRECT_HIRE', 'AGENCY_HOME_CARE', 'AGENCY_FACILITY') THEN
    ...
    -- Create department with department_code
    INSERT INTO departments (id, name, agency_id, supervisor_id, status, department_code)
    VALUES ('d0000000-0000-0000-0000-000000000001', 'Personal Care', v_agency_id, v_supervisor_user_id, 'active', 'PERSCARE')
    ON CONFLICT (id) DO UPDATE SET supervisor_id = EXCLUDED.supervisor_id, department_code = EXCLUDED.department_code
