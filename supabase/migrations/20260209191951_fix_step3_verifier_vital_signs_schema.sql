/*
  # Fix Step 3 Verifier - Correct Vital Signs Schema
  
  Updates verifier to use correct vital_signs table schema
*/
...
BEGIN
  ...
  IF v_test_resident_id IS NULL THEN
    ...
    RETURN jsonb_build_object('overall_status', v_overall_status, 'agency_id', p_agency_id, 'checks', v_checks, 'executed_at', now());
  END IF;
  ...
  IF v_test_medication_id IS NULL THEN
    INSERT INTO resident_medications (resident_id, medication_name, dosage, route, frequency, prescriber, is_active, is_controlled, is_prn)
    VALUES (v_test_resident_id, 'Test BP Med', '10mg', 'ORAL', 'BID', 'Dr. Test', true, false, false)
  ...
  v_time_diff_minutes := 120;

  INSERT INTO medication_administration_log (resident_id, medication_id, scheduled_time, administered_at, administered_by, status, dosage_given, route_used, is_controlled, dual_verification_required, dual_verification_completed, language_context, is_simulation)
  VALUES (v_test_resident_id, v_test_medication_id, v_scheduled_time, v_administered_at, v_test_user_id, 'LATE', '10mg', 'ORAL', false, false, false, 'en', true)
  RETURNING id INTO v_test_log_id;

  v_checks := array_append(v_checks, jsonb_build_object('check', 'late_medication_signal_1', 'status', 'PASS', 'message', format('Generated late medication signal (log_id: %s, 120 min late)', v_test_log_id)));

  -- CHECK 4: Generate LATE medication signal 2
  v_scheduled_time := now() - interval '90 minutes';
  INSERT INTO medication_administration_log (resident_id, medication_id, scheduled_time, administered_at, administered_by, status, dosage_given, route_used, is_controlled, dual_verification_required, dual_verification_completed, language_context, is_simulation)
  VALUES (v_test_resident_id, v_test_medication_id, v_scheduled_time, now(), v_test_user_id, 'LATE', '10mg', 'ORAL', false, false, false, 'en', true)
  RETURNING id INTO v_test_log_id;

  v_checks := array_append(v_checks, jsonb_build_object('check', 'late_medication_signal_2', 'status', 'PASS', 'message', format('Generated second late medication signal (log_id: %s)', v_test_log_id)));

  -- CHECK 5: Generate ABNORMAL vital sign signal
  INSERT INTO vital_signs (resident_id, vital_type, systolic, diastolic, recorded_at, recorded_by, notes, is_simulation, idempotency_key)
  VALUES (v_test_resident_id, 'blood_pressure', 185, 95, now(), v_test_user_id, 'Feeling pressure in head', true, v_idempotency_base || '_vital_1')
  RETURNING id INTO v_test_log_id;

  v_checks := array_append(v_checks, jsonb_build_object('check', 'abnormal_vital_signal', 'status', 'PASS', 'message', format('Generated abnormal vital signal (BP: 185/95, vital_id: %s)', v_test_log_id)));

  -- CHECK 6: Generate FAMILY observation signal
  INSERT INTO observation_events (resident_id, observer_type, observer_id, event_type, observation_text, severity, quality, confidence_level, requires_followup, is_simulation)
  VALUES (v_test_resident_id, 'FAMILY', v_test_user_id, 'COGNITIVE', 'Mom seems confused and forgetful. She missed her morning medications.', 'MODERATE', 'HIGH', 0.85, true, true)
  RETURNING id INTO v_test_log_id;

  v_checks := array_append(v_checks, jsonb_build_object('check', 'family_observation_signal', 'status', 'PASS', 'message', format('Generated family observation signal (obs_id: %s)', v_test_log_id)));
  ...
  ELSE
    v_checks := array_append(v_checks, jsonb_build_object('check', 'correlation_engine_executed', 'status', 'FAIL', 'message', format('Correlation engine failed: %s', v_correlation_result)));
    v_overall_status := 'FAIL';
  END IF;
  ...
  ELSE
    v_checks := array_append(v_checks, jsonb_build_object('check', 'compound_event_created', 'status', 'FAIL', 'message', 'No compound intelligence event was created'));
    v_overall_status := 'FAIL';
  END IF;
  ...
  IF v_compound_event_id IS NOT NULL THEN
    ...
    ELSE
      v_checks := array_append(v_checks, jsonb_build_object('check', 'signal_contributions_linked', 'status', 'FAIL', 'message', format('Insufficient signal contributions: %s', v_signal_count)));
      v_overall_status := 'FAIL';
    END IF;
  ELSE
    v_checks := array_append(v_checks, jsonb_build_object('check', 'signal_contributions_linked', 'status', 'FAIL', 'message', 'Cannot check: no compound event'));
    v_overall_status := 'FAIL';
  END IF;
  ...
  IF v_compound_event_id IS NOT NULL THEN
    ...
    ELSE
      v_checks := array_append(v_checks, jsonb_build_object('check', 'explainability_complete', 'status', 'FAIL', 'message', 'Explainability incomplete'));
      v_overall_status := 'FAIL';
    END IF;
  ELSE
    v_checks := array_append(v_checks, jsonb_build_object('check', 'explainability_complete', 'status', 'FAIL', 'message', 'Cannot check: no compound event'));
    v_overall_status := 'FAIL';
  END IF;
  ...
  IF v_compound_event_id IS NOT NULL THEN
    ...
    ELSE
      v_checks := array_append(v_checks, jsonb_build_object('check', 'timeline_entry_created', 'status', 'FAIL', 'message', 'Timeline entry not created'));
      v_overall_status := 'FAIL';
    END IF;
  ELSE
    v_checks := array_append(v_checks, jsonb_build_object('check', 'timeline_entry_created', 'status', 'FAIL', 'message', 'Cannot check: no compound event'));
    v_overall_status := 'FAIL';
  END IF;
