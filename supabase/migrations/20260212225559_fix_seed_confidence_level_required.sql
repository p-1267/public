/*
  # Fix seed_active_context confidence_level NOT NULL

  **BLOCKER:** health_metrics.confidence_level is NOT NULL, must provide value
  **FIX:** Add confidence_level to INSERT statement
*/
...
BEGIN
  ...
  FOR i IN 0..4 LOOP
    ...
    VALUES
      (v_resident.id, 'VITAL', 'blood_pressure_systolic', 120 + (i * 2), 'mmHg', now() - (i || ' days')::interval, 'MANUAL', 'HIGH'),
      (v_resident.id, 'VITAL', 'heart_rate', 72 + i, 'bpm', now() - (i || ' days')::interval, 'MANUAL', 'HIGH')
    ON CONFLICT DO NOTHING;
  END LOOP;
