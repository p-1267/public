/*
  # Phase 29 Entry Gate RPC (Phase 29)

  ## Purpose
  Checks if user can enter Phase 29 (Payments, Credentials & External Key Sequencing).
  Enforces hard gate conditions.

  ## Entry Conditions (ALL must be satisfied)
  1. Organization state = ACTIVE
  2. Phases 18-28 = COMPLETED
  3. User role = AGENCY_ADMIN or SUPER_ADMIN

  ## Function
  - check_phase29_entry_gate() - Returns allow/block with detailed reasons
...
-- Function: check_phase29_entry_gate
-- Checks if current user can enter Phase 29
CREATE OR REPLACE FUNCTION check_phase29_entry_gate()
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_user_role NOT IN ('AGENCY_ADMIN', 'SUPER_ADMIN') THEN
    v_blocked_reasons := array_append(v_blocked_reasons, 'ROLE_NOT_AUTHORIZED: Only AGENCY_ADMIN or SUPER_ADMIN can manage credentials');
    v_can_enter := false;
  END IF;
  ...
  -- Phases 19-28 are implicitly completed if organization is active
