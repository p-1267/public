/*
  # Add confidence_level to health_metrics inserts
*/
...
BEGIN
  ...
  FOR i IN 0..4 LOOP
    ...
    INSERT INTO health_metrics (resident_id, metric_category, metric_type, value_numeric, unit, recorded_at, measurement_source, data_source, confidence_level) VALUES (v_resident.id, 'VITAL', 'blood_pressure_systolic', 120 + i*2, 'mmHg', now() - (i || ' days')::interval, 'MANUAL', 'MANUAL', 'HIGH'), (v_resident.id, 'VITAL', 'heart_rate', 72 + i, 'bpm', now() - (i || ' days')::interval, 'MANUAL', 'MANUAL', 'HIGH') ON CONFLICT DO NOTHING;
  END LOOP;
