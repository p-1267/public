/*
  # Fix all remaining table schema mismatches

  Fixes:
  - departments: Add department_code, remove is_active, use status
  - task_categories: Add category_type (required)
  - resident_medications: Use prescriber_name, add schedule jsonb
  - vital_signs: value is text, use recorded_at/recorded_by, no unit/device_id
  - appointments: Add title (required)
  - resident_documents: Add title, storage_path, remove document_type/file_url
*/
...
BEGIN
  ...
  VALUES
    (v_resident.id, 'Care Plan', 'care-plan.pdf', '/documents/care-plan.pdf', COALESCE(v_admin_user_id, v_caregiver_user_id), now() - interval '30 days'),
    (v_resident.id, 'Medical History', 'medical-history.pdf', '/documents/medical-history.pdf', COALESCE(v_admin_user_id, v_caregiver_user_id), now() - interval '60 days')
  ON CONFLICT DO NOTHING;
