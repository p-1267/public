/*
  ...
  This migration fixes the seed_senior_family_scenario function to match actual database schema:
  1. resident_documents: file_size instead of file_size_bytes
  2. resident_documents: correct column names (title, file_name, etc.)
  3. lab_tests: correct schema (no test_category, result_value, etc.)
  4. notification_log: correct column names (priority, no delivery_status/sent_at)
  5. Remove invalid column references
*/
...
BEGIN
  ...
  RETURN jsonb_build_object('status', 'SUCCESS', 'message', 'Senior + Family data seeded - schema corrected', 'resident_id', v_resident_id, 'functional_pages', '22 of 22', 'data', jsonb_build_object('medications', 2, 'appointments', 3, 'lab_tests', 3, 'documents', 3, 'care_plan', 1, 'emergency_contacts', 3, 'notifications', 5, 'vital_signs', v_vital_count, 'health_metrics', (SELECT COUNT(*) FROM health_metrics WHERE resident_id = v_resident_id), 'health_trends', (SELECT COUNT(*) FROM health_metric_trends WHERE resident_id = v_resident_id)));
END;
$$;