/*
  # Seed with NULL created_by
  
  Uses NULL for created_by since showcase users don't exist in auth.users
*/
...
BEGIN
  ...
  -- 4. Create Task Templates (created_by = NULL for showcase)
  ...
  -- 5. Create Today's Tasks for both residents (created_by = NULL for showcase)
  FOR i IN 1..2 LOOP
    INSERT INTO tasks (
  ...
  INSERT INTO vital_signs (resident_id, recorded_at, recorded_by, systolic_bp, diastolic_bp, heart_rate, 
                           respiratory_rate, temperature_f, oxygen_saturation, blood_glucose, notes, created_at)
  VALUES
    (v_resident_ids[1], now() - interval '3 hours', v_caregiver_id, 128, 82, 72, 16, 98.4, 97, null,
     'All vitals within normal range. Resident alert and responsive.', now()),
    (v_resident_ids[1], now() - interval '1 day 3 hours', v_caregiver_id, 132, 84, 75, 16, 98.2, 96, null,
     'Stable readings. No concerns.', now()),
    (v_resident_ids[2], now() - interval '3 hours', v_caregiver_id, 118, 76, 68, 14, 98.6, 98, 105,
