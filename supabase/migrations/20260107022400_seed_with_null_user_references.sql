/*
  # Seed with NULL User References
  
  Updates seed function to use NULL for all user foreign keys
  since showcase users don't exist in auth.users
*/
...
BEGIN
  ...
  VALUES (
    v_agency_id, 'Demo Care Agency', 'active', 'AGENCY',
  ...
  FOR i IN 1..2 LOOP
    INSERT INTO tasks (
      ...
      (v_agency_id, v_resident_ids[i], 'Morning Medication', 'NURSING', 'high',
       ...
       v_category_nursing, v_template_morning_med, 'CAREGIVER',
      ...
      (v_agency_id, v_resident_ids[i], 'Vital Signs Check', 'NURSING', 'medium',
       ...
       v_category_nursing, v_template_vitals, 'CAREGIVER',
       true, false, false, false, false, now(), now()),
      (v_agency_id, v_resident_ids[i], 'Breakfast Delivery', 'KITCHEN', 'high',
       ...
       v_category_kitchen, v_template_breakfast, 'CAREGIVER',
       false, false, true, false, false, now(), now()),
      (v_agency_id, v_resident_ids[i], 'Room Cleaning', 'HOUSEKEEPING', 'medium',
       ...
       v_category_housekeeping, v_template_room_clean, 'CAREGIVER',
  ...
  -- Update completed tasks with actual times
  UPDATE tasks 
  SET 
    actual_start = scheduled_start - interval '2 minutes',
    actual_end = scheduled_end - interval '2 minutes',
    outcome = 'success'
  ...
  INSERT INTO intelligence_signals (
    ...
    detected_at, evidence, reasoning, recommended_actions, created_at
  ) VALUES
    (v_resident_ids[1], 'anomaly', 'Unusual Medication Timing', 'medium', 0.85, now() - interval '2 hours',
     ...
     '["Review caregiver morning schedule", "Consider adjusting medication time window"]'::jsonb, now()),
    (v_resident_ids[1], 'trend', 'Declining Meal Intake', 'high', 0.78, now() - interval '1 hour',
     ...
     '["Consult with dietitian", "Review meal preferences", "Consider nutritional supplements"]'::jsonb, now()),
    (v_resident_ids[2], 'positive', 'Improved Mobility', 'low', 0.92, now() - interval '30 minutes',
     ...
     '["Continue current PT regimen", "Consider graduated increase in activity"]'::jsonb, now());
  
  -- 7. Create Vital Signs (recorded_by = NULL for showcase)
  INSERT INTO vital_signs (resident_id, recorded_at, systolic_bp, diastolic_bp, heart_rate, 
                           respiratory_rate, temperature_f, oxygen_saturation, blood_glucose, notes, created_at)
  VALUES
