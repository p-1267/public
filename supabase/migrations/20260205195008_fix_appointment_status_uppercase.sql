/*
  # Fix appointment status to use uppercase (CHECK constraint requirement)
*/
...
BEGIN
  ...
  INSERT INTO lab_tests (resident_id, test_name, test_category, ordered_date, completed_date, ordered_by, status, result_value, result_unit, reference_range) VALUES
  ...
  INSERT INTO resident_documents (resident_id, document_name, document_type, upload_date, uploaded_by, file_size_bytes) VALUES
  ...
  INSERT INTO resident_care_plan_anchors (resident_id, care_focus_area, goals, interventions, review_frequency) VALUES (v_resident_id, 'Diabetes Management', '["Maintain HbA1c below 7.0%", "Monitor blood sugar daily"]', '["Take Metformin as prescribed", "Follow diabetic diet plan", "Regular exercise 30min/day"]', 'MONTHLY') ON CONFLICT (resident_id, care_focus_area) DO UPDATE SET goals = EXCLUDED.goals;

  DELETE FROM resident_emergency_contacts WHERE resident_id = v_resident_id;
  INSERT INTO resident_emergency_contacts (resident_id, contact_name, relationship, phone_primary, phone_secondary, is_primary_contact, contact_order) VALUES
  ...
  INSERT INTO notification_log (user_id, resident_id, notification_type, title, message, delivery_method, delivery_status, sent_at) VALUES
  ...
  RETURN jsonb_build_object('status', 'SUCCESS', 'message', 'Senior + Family data seeded with auto-calculated health trends', 'resident_id', v_resident_id, 'functional_pages', '22 of 22', 'data', jsonb_build_object('medications', 2, 'appointments', 3, 'lab_tests', 3, 'documents', 3, 'care_plan', 1, 'emergency_contacts', 3, 'notifications', 5, 'vital_signs', v_vital_count, 'health_metrics', (SELECT COUNT(*) FROM health_metrics WHERE resident_id = v_resident_id), 'health_trends', (SELECT COUNT(*) FROM health_metric_trends WHERE resident_id = v_resident_id)));
END;
$$;
