/*
  # Fix seed_active_context tasks INSERT to match actual schema

  ## Changes
  - Use category_id (uuid) instead of category (text)
  - Use task_name instead of title
  - Use scheduled_start and scheduled_end instead of scheduled_for
  - Add required agency_id column
  - Fix column names to match actual tasks table

  ## Fixes
  - ERROR: column "category" of relation "tasks" does not exist
*/
...
BEGIN
  ...
  IF v_context.service_model IN ('DIRECT_HIRE', 'AGENCY_HOME_CARE', 'AGENCY_FACILITY') THEN
    ...
    -- Assign caregiver to resident (include agency_id)
  ...
  -- Get or create task categories
  INSERT INTO task_categories (id, name, category_label)
  VALUES
    ('c0000000-0000-0000-0000-000000000001', 'medication', 'Medication Administration'),
    ('c0000000-0000-0000-0000-000000000002', 'vital_signs', 'Vital Signs Check'),
    ('c0000000-0000-0000-0000-000000000003', 'meal', 'Meal Assistance')
  ON CONFLICT (id) DO NOTHING;

  SELECT id INTO v_category_med_id FROM task_categories WHERE name = 'medication' LIMIT 1;
  SELECT id INTO v_category_vitals_id FROM task_categories WHERE name = 'vital_signs' LIMIT 1;
  SELECT id INTO v_category_meal_id FROM task_categories WHERE name = 'meal' LIMIT 1;
  ...
  IF v_caregiver_user_id IS NOT NULL AND v_category_med_id IS NOT NULL THEN
    ...
    VALUES
      (v_agency_id, v_resident.id, v_category_med_id, 'Administer morning medications', 'Lisinopril 10mg, Metformin 500mg', 'ready', 'high', 'B', CURRENT_TIMESTAMP + INTERVAL '1 hour', CURRENT_TIMESTAMP + INTERVAL '1 hour 30 minutes', v_caregiver_user_id),
      (v_agency_id, v_resident.id, v_category_vitals_id, 'Check blood pressure', 'Morning vital signs check', 'ready', 'medium', 'B', CURRENT_TIMESTAMP + INTERVAL '30 minutes', CURRENT_TIMESTAMP + INTERVAL '1 hour', v_caregiver_user_id),
      (v_agency_id, v_resident.id, v_category_meal_id, 'Assist with breakfast', 'Ensure proper nutrition intake', 'ready', 'medium', 'B', CURRENT_TIMESTAMP + INTERVAL '2 hours', CURRENT_TIMESTAMP + INTERVAL '2 hours 30 minutes', v_caregiver_user_id)
    ON CONFLICT DO NOTHING;
  END IF;
