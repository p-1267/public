/*
  # Fix Showcase Seed - Correct Schema
...
BEGIN
  ...
  -- Create task categories (using category_type not department)
  INSERT INTO task_categories (agency_id, name, category_type, description, default_priority, requires_evidence, metadata)
  VALUES
    (v_agency_id, 'Medication Administration', 'medication', 'Administer prescribed medications', 'high', true, '{"department": "NURSING"}'::jsonb),
    (v_agency_id, 'Vital Signs Monitoring', 'assessment', 'Monitor and record vital signs', 'medium', true, '{"department": "NURSING"}'::jsonb),
    ...
    (v_agency_id, 'Meal Preparation', 'nutrition', 'Prepare meals according to dietary requirements', 'high', true, '{"department": "KITCHEN"}'::jsonb),
