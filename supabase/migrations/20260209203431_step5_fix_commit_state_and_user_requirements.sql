/*
  # Fix Voice Commit Function - State and User Requirements
  
  Fixes:
  - tasks.state must be 'scheduled' (not 'pending')
  - administered_by is NOT NULL, need valid user_id
*/
...
BEGIN
  ...
  -- Get or create system user for voice actions
  SELECT id INTO v_system_user_id FROM user_profiles WHERE email = 'system@voice.internal' LIMIT 1;
  IF v_system_user_id IS NULL THEN
    SELECT id INTO v_system_user_id FROM user_profiles LIMIT 1;
  END IF;
  
  v_user_id_for_insert := COALESCE(p_user_id, v_system_user_id);
