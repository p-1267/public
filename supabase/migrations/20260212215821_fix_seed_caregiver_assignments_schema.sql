/*
  # Fix seed_active_context caregiver_assignments schema

  Updates INSERT to match actual caregiver_assignments table schema.
  Columns: id, agency_id, resident_id, caregiver_user_id, assigned_by, status
*/
...
BEGIN
  ...
  IF v_context.service_model IN ('DIRECT_HIRE', 'AGENCY_HOME_CARE', 'AGENCY_FACILITY') THEN
    ...
    INSERT INTO caregiver_assignments (agency_id, resident_id, caregiver_user_id, status)
    VALUES (v_agency_id, v_resident.id, v_caregiver_user_id, 'active') ON CONFLICT DO NOTHING;
  END IF;
