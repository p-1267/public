/*
  # User Profiles Audit Trigger

  1. Purpose
    - Automatically log all user profile changes
    - Captures role changes, activation/deactivation
    - Records who made the change

  2. Trigger Details
    - Fires AFTER INSERT, UPDATE, DELETE on user_profiles
    - Logs role assignments, status changes
    - Distinguishes between self-updates and admin updates

  3. Security
    - Trigger function uses SECURITY DEFINER
    - Ensures accountability for user management actions
*/
...
CREATE TRIGGER audit_user_profile_changes
  ...
  FOR EACH ROW
  EXECUTE FUNCTION trigger_audit_user_profile_change();