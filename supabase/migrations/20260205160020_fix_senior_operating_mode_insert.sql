/*
  # Fix Senior Operating Mode Insert
  
  The table allows multiple records per resident (temporal design).
  Remove ON CONFLICT - just insert new mode record each time.
*/
...
BEGIN
  ...
  -- Set operating mode (temporal design - always insert, don't use ON CONFLICT)
  -- First disable any existing active modes
