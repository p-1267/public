/*
  # Fix seed_active_context measurement_source enum

  **BLOCKER:** CHECK constraint requires 'MANUAL_ENTRY' not 'MANUAL'
  **FIX:** Update measurement_source to match constraint
*/
...
BEGIN
  ...
  FOR i IN 0..4 LOOP
    ...
    -- FIXED: Use MANUAL_ENTRY not MANUAL
    INSERT INTO health_metrics (
      resident_id, metric_category, metric_type, value_numeric,
      unit, recorded_at, measurement_source, confidence_level
    )
    VALUES
      (v_resident.id, 'VITAL', 'blood_pressure_systolic', 120 + (i * 2), 'mmHg', now() - (i || ' days')::interval, 'MANUAL_ENTRY', 'HIGH'),
      (v_resident.id, 'VITAL', 'heart_rate', 72 + i, 'bpm', now() - (i || ' days')::interval, 'MANUAL_ENTRY', 'HIGH')
    ON CONFLICT DO NOTHING;
  END LOOP;
