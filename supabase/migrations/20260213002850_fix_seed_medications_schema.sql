/*
  # Fix seed_active_context medications schema
  
  ## Changes
  - Use prescriber_name instead of prescribed_by
  - Add required fields: schedule, is_prn, is_controlled, start_date
*/
...
BEGIN
  ...
  -- Create medications with correct schema
  INSERT INTO resident_medications (resident_id, medication_name, dosage, frequency, route, schedule, prescriber_name, is_prn, is_controlled, start_date, is_active)
  VALUES
    (v_resident.id, 'Lisinopril', '10mg', 'DAILY', 'ORAL', '{"times": ["09:00"]}'::jsonb, 'Dr. Smith', false, false, CURRENT_DATE, true),
    (v_resident.id, 'Metformin', '500mg', 'TWICE_DAILY', 'ORAL', '{"times": ["09:00", "21:00"]}'::jsonb, 'Dr. Smith', false, false, CURRENT_DATE, true),
    (v_resident.id, 'Atorvastatin', '20mg', 'DAILY', 'ORAL', '{"times": ["21:00"]}'::jsonb, 'Dr. Jones', false, false, CURRENT_DATE, true)
  ON CONFLICT DO NOTHING;
