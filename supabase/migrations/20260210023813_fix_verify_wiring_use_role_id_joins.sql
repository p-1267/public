-- Fix verifier to use role_id joins
...
BEGIN
  ...
  SELECT id INTO v_caregiver_user_id FROM user_profiles WHERE role_id = v_caregiver_role_id AND agency_id = v_agency_id LIMIT 1;
  SELECT id INTO v_supervisor_user_id FROM user_profiles WHERE role_id = v_supervisor_role_id AND agency_id = v_agency_id LIMIT 1;
  ...
  IF v_agency_id IS NULL THEN
    RETURN QUERY SELECT 'SENIOR_FAMILY'::text, 'ALL'::text, 'SEED'::text,
      ...
      'FAIL'::text, 'No agency seeded'::text;
    RETURN;
  END IF;
  ...
  BEGIN
    ...
    RETURN QUERY SELECT 'SENIOR_FAMILY'::text, 'FAMILY_VIEWER'::text, 'home'::text,
      ...
      format('resident=%s, family_link=%s', v_resident_id, v_family_user_id IS NOT NULL)::text,
