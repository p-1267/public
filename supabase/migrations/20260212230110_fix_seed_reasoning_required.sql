/*
  # Fix seed_active_context - add reasoning to intelligence_signals

  **BLOCKER:** reasoning is NOT NULL in intelligence_signals
  **FIX:** Add reasoning column to INSERT
*/
...
BEGIN
  ...
  IF v_entered_by IS NULL THEN
    RAISE EXCEPTION 'seed_active_context: cannot resolve entered_by for care_context_id=%', p_care_context_id;
  END IF;

  INSERT INTO resident_medications (
    resident_id, medication_name, dosage, frequency, route, schedule,
    prescriber_name, is_prn, is_controlled, start_date, is_active,
    entered_by, language_context
  )
  VALUES
    (v_resident.id, 'Lisinopril', '10 mg', 'Once daily', 'ORAL',
     '{"times": ["09:00"]}'::jsonb, 'Dr. Sarah Johnson',
     false, false, CURRENT_DATE, true, v_entered_by, 'Clinical medication order'),
    (v_resident.id, 'Metformin', '500 mg', 'Twice daily', 'ORAL',
     '{"times": ["09:00", "21:00"]}'::jsonb, 'Dr. Sarah Johnson',
     false, false, CURRENT_DATE, true, v_entered_by, 'Clinical medication order')
  ON CONFLICT DO NOTHING;
  ...
  FOR i IN 0..4 LOOP
    ...
    VALUES
      (v_resident.id, 'BLOOD_PRESSURE_SYSTOLIC', (120 + (i * 2))::text, now() - (i || ' days')::interval, v_entered_by),
      (v_resident.id, 'HEART_RATE', (72 + i)::text, now() - (i || ' days')::interval, v_entered_by)
    ON CONFLICT DO NOTHING;

    INSERT INTO health_metrics (
      resident_id, metric_category, metric_type, value_numeric,
      unit, recorded_at, measurement_source, confidence_level
    )
    VALUES
      (v_resident.id, 'BLOOD_PRESSURE', 'blood_pressure_systolic', 120 + (i * 2), 'mmHg', now() - (i || ' days')::interval, 'MANUAL_ENTRY', 'HIGH'),
      (v_resident.id, 'CARDIOVASCULAR', 'heart_rate', 72 + i, 'bpm', now() - (i || ' days')::interval, 'MANUAL_ENTRY', 'HIGH')
    ON CONFLICT DO NOTHING;
  END LOOP;
  ...
  VALUES
    ...
    (v_resident.id, 'SCREENING', 'Blood Work', now() + interval '14 days', 30, 'SCHEDULED', 'LabCorp', 'Lab Center')
  ON CONFLICT DO NOTHING;

  INSERT INTO intelligence_signals (
    resident_id, agency_id, signal_id, category, title,
    description, reasoning, severity, detected_at, requires_human_action
  )
  VALUES
    (v_resident.id, v_agency_id, 'vital-trend-001', 'HEALTH', 'Vital Trend Alert',
     'Blood pressure trending upward', 'Systolic readings increased 10 points over 5 days', 'MEDIUM', now(), true),
    (v_resident.id, v_agency_id, 'activity-change-001', 'BEHAVIOR', 'Activity Change',
     'Decreased daily activity', 'Step count down 30% compared to baseline', 'LOW', now(), false)
  ON CONFLICT DO NOTHING;
