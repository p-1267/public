/*
  # Fix department selection to avoid multiple rows

  Use SELECT after INSERTs instead of RETURNING when querying multiple rows.
*/

DROP FUNCTION IF EXISTS seed_senior_family_scenario();

CREATE FUNCTION seed_senior_family_scenario()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  SELECT id INTO v_dept_dietary_id FROM departments WHERE agency_id = v_agency_id AND department_code = 'DIETARY';
