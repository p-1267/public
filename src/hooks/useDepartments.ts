...
export const useDepartments = () => {
  ...
  const loadDepartments = useCallback(async () => {
    ...
    try {
      ...
      console.log('[useDepartments] Calling RPC get_agency_departments...');
      const { data, error: rpcError } = await supabase.rpc('get_agency_departments', { p_agency_id: agencyId });
...
export const useDepartmentPersonnel = (departmentId?: string) => {
  ...
  const loadPersonnel = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc('get_department_personnel', {
...
export const useDepartmentAssignments = (departmentId?: string, assignedToId?: string) => {
  ...
  const loadAssignments = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc('get_department_assignments', {
        p_agency_id: agencyId,
        p_department_id: departmentId || null,
...
export const useDepartmentSchedules = (departmentId?: string, shiftDate?: string) => {
  ...
  const loadSchedules = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc('get_department_schedules', {
        p_agency_id: agencyId,
        p_department_id: departmentId || null,
